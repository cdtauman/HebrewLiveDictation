...
- **.NET SDK pin (`.NET 9`, learned from the first CI run):** WindowsAppSDK 1.7 PRI generation
  (`MrtCore.PriGen`) is incompatible with the runner's **preinstalled .NET 10 SDK** — `dotnet publish`
  dies with `MSB4062` (the `ExpandPriContent` task DLL is absent from the .NET 10 SDK's `AppxPackage`
  folder). `setup-dotnet` installs 9.0.x, but the highest installed SDK (10) wins unless pinned, so the
  workflow writes a **workspace-only `global.json`** (`version 9.0.100`, `rollForward: latestFeature`)
  before building — selecting the highest 9.0.x and never rolling to 10. It is **not committed**
  (gitignored): local builds work on .NET 9 *or* .NET 10 (where VS supplies the PriGen task), so the repo
  does not force a specific local SDK. Verified locally: `dotnet publish` exits 0 on .NET 9.0.315 with
  the pin (the same publish fails on the runner's .NET 10).
